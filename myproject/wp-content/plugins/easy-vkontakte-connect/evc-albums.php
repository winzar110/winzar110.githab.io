...
function evc_vk_albums_vk_photos_get_all( $params ) {
	...
	//evc_add_log( 'evc_vk_albums_photos_get_all: ' .print_r($args,1) );//
...
function evc_vk_albums_vk_photos_get( $params ) {
	...
	//evc_add_log( 'evc_vk_albums_photos_get: ' .print_r($args,1) );//
...
function evc_vk_albums_shorcode( $data ) {
	...
	if ( ! empty( $data ) && ! empty( $data['url'] ) ) {
		...
		$out = '[collage_gallery ' . implode( ' ', $_out ) . ']';
	}
...
function evc_vk_albums_evc_meta_box_action( $custom ) {
	...
	$evc_vk_album['insert_in'] = wp_parse_args( $evc_vk_album['insert_in'], $defaults['insert_in'] );
	...
	<p>
		...
		<br/><label><input type="checkbox" name="evc_vk_album[insert_in][singular]" <?php echo checked( $evc_vk_album['insert_in']['singular'], 'on', false ); ?> >страницах записей,
			<small>is_singular()</small>
		</label>
