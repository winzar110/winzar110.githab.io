...
function evc_vk_api_settings_admin_init() {
  ...
  <p>Если приложение с этими настройками у вас было создано ранее, вы можете найти его на <a href="https://vk.com/apps?act=manage" target="_blank">странице приложений</a> и, нажав "Редактировать", найти его ID.</p>';
	...
	$fields = array(
        ...
        <br/><br/>Вы можете создать <a href="http://vk.com/public.php?act=new" target="_blank">новую страницу</a> ВКонтакте или найти среди ваших уже <a href="http://vk.com/public.php?act=newY" target="_blank">созданных страниц</a>.', 'evc' ),
			...
			array(
				...
				<br>Если значение все еще не появилось, нужно открыть <a href="'.admin_url('/admin.php&page=evc-log').'">Лог плагина</a>, там могут отображаться возможные ошибки.
				<br>Если из Лога неясно в чем дело, можно написать в <a href="https://vk.me/wordpressvk">службу поддержки</a>.', 'evc' ),
				'type'     => 'text',
				'readonly' => true
			),
			array(
...
<br>Дата обязательно должна быть указана в формате: <code>ГГГГ-ММ-ДД ЧЧ:ММ</code>, например,
        ...
        <br/>API ВКонтакте не позволяет прикреплять больше 6 фото.
        ...
        <br/><code>%author%</code> - автор записи,
        ...
        <br/>
        <br/><small>Доступно в <a href = "javascript:void(0);" class = "get-evc-pro">PRO версии</a>.</small>
        <br/><code>%tags%</code> - метки (теги) записи - в локальные хештеги ВК (<code>#метка@groupScreenName</code>),
        <br/><code>%cats%</code> - рубрики записи - в локальные хештеги ВК,
        <br/><code>%meta_tags%</code> - метки (теги) записи - в глобальные хештеги ВК (<code>#метка</code>),
        <br/><code>%meta_cats%</code> - рубрики записи - в глобальные хештеги ВК,
        <br>Преобразование меток и рубрик в <b>локальные хештеги</b> ВК может повысить количество просмотров материалов посетителями группы. <b>Глобальные хештеги</b> могут привлечь новых посетителей из поиска.
        <br/><br/><code>%vk_author%</code> - автор записи гиперссылкой на профиль в ВК, например, <code>@id1(display_name)</code>, где
        <br><b>id1</b> - id автора в ВК (если вход на сайт был выполнен через кнопку Войти через ВКонтакте),
        <br><b>display_name</b> - отображаемое имя автора.
        ', 'evc' ),
				'type'    => 'textarea',
				'default' => "%title%\n\n%excerpt%"
			)
...
function evc_ad() {
	...
	<div class="evc-boxx">
		<p><a href="http://ukraya.ru/428/easy-vkontakte-connect-evc" target="_blank">Помощь</a> по работе с плагином
			<em>Easy VKontakte Connect</em>. <a href="http://ukraya.ru/428/easy-vkontakte-connect-evc" target="_blank">Доработка</a> под ваши задачи и <a href="https://vk.me/wordpressvk" target="_blank">техническая поддержка</a>.</p>
	</div>
	...
	if ( ! $is_collage_gallery ) {
		...
		<div class="evc-boxx">
			...
			<p>
				<a class=" button button-primary" href="<?php echo admin_url( 'plugin-install.php?s=Collage+Gallery+Aleksej+Solovjov&tab=search&type=term' ); ?>" target="_blank">Установить сейчас</a>
			</p>
		</div>
		<?php
	}
	...
	<div class="evc-boxx">
		<p><b>Разработка проектов любой сложности на WordPress</b> от создателей плагина.
			<a href="https://vk.me/wordpressvk" target="_blank">Написать!</a></p>
	</div>
	...
	if ( ! function_exists( 'vkm_init' ) ) {
		...
		<p>
			<a class="button button-primary"href="<?php echo admin_url( 'plugin-install.php?s=VKMarket+for+WooCommerce+Aleksej+Solovjov&tab=search&type=term' ); ?>" target="_blank">Установить бесплатно</a>
		</p>
		<?php
	}
	...
	if ( ! $is_collage_gallery ) {
		...
		<p>
			<a class="button button-primary" href="<?php echo admin_url( 'plugin-install.php?s=Collage+Gallery+Aleksej+Solovjov&tab=search&type=term' ); ?>" target="_blank">Установить бесплатно</a>
		</p>
		<?php
	}
	...
	<p>
		...
		<br>Если не сложно, пожалуйста,
		<a href="https://wordpress.org/support/plugin/easy-vkontakte-connect/reviews/?rate=5#new-post">оцените плагин</a> в &#9733;&#9733;&#9733;&#9733;&#9733; на его странице в WordPress.
	</p>
	<?php
...
function evc_share_vk_autorization() {
	...
	if (  !empty($_GET['page']) && 'evc' == $_GET['page'] && false !== ( $token = evc_share_get_token() ) ) {
...
function evc_share_get_token() {
	...
	if ( !empty($_GET['page']) && 'evc' == $_GET['page'] && isset( $_GET['code'] ) && ! empty( $_GET['code'] ) ) {
