...
function evc_update_user( $user_id, $data ) {
	...
	$udata = array(
		'ID'           => $user_id,
		'user_url'     => 'http://vk.com/id' . $data['id'],
		'display_name' => $data['first_name'] . ' ' . $data['last_name'],
		'first_name'   => $data['first_name'],
		'last_name'    => $data['last_name'],
	);
...
if ( ! function_exists( 'pluralize' ) ) :
	function pluralize( $count, $singular, $plural = false, $pluralmore = false ) {
		...
		if ( $last_two_digits < 10 || $last_two_digits > 20 ) {
			if ( $last_digits == 1 ) {
				return $singular;
			} elseif ( in_array( $last_digits, array( 2, 3, 4 ) ) ) {
				return $plural;
			}
		}
...
function evc_get_lock( $transient ) {
	global $wpdb;
...
function evc_vkapi_requests_limit() {
	$vkapi1 = microtime( true );
	//$vkapi = get_transient('vkapi');
	$vkapi = evc_get_lock( 'vkapi' );
...
function evc_get_vk_object( $url ) {
	$out = array(); // $out = ''; // $out = array(); for php7 http://php.net/manual/ru/language.types.array.php
