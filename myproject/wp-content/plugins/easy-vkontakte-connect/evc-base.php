...
function evc_plugin_loader() {
	...
	include_once( 'evc-widgets.php' );
	include_once( 'evc-bulk.php' );
}
...
function evc_admin_enqueue_scripts( $hook ) {
	...
	wp_enqueue_script( 'evc', plugins_url( 'js/evc.js', __FILE__ ), array(
		'jquery',
		'jquery-masonry',
		'sticky-kit'
	), '1.0', true );
...
function evc_wall_post( $id, $post ) {
	...
	$permalink = ! empty( $options['format']['add_link'] ) ? apply_filters( 'evc_publish_permalink', wp_get_shortlink( $post->ID ), $post->ID ) : '';

	// In order to use IDN validation intl extension must be installed and enabled.
	if ( ! empty( $permalink ) && function_exists( 'idn_to_ascii' ) ) {
		$permalink = preg_replace_callback( '/:\/\/([^\/]+)/', function ( $matches ) {
	...
	// Wall Post with link
	if ( ! empty( $resp['response']['processing'] ) || ! empty( $resp['response']['post_id'] ) ) {
...
function evc_upload_photo( $id, $post ) {
	...
	if ( $options['upload_photo_count'] > 6 ) {
		$options['upload_photo_count'] = 6;
	}
